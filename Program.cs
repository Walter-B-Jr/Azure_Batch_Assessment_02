    ...
    public class Program
    {
        ...
        private static async Task MainAsync()
        {
            ...
            using (BatchClient batchClient = BatchClient.Open(cred))
            {
                ...
                await CreateJobAsync(batchClient, JobId, PoolId, inputFiles);
        ...
        private static async Task<List<CloudTask>> AddTasksAsync(BatchClient batchClient, string jobId, List<ResourceFile> inputFiles, string outputContainerSasUrl)
        {
            ...
            {
                string taskId = "assessment_test_task_" + inputFiles.IndexOf(inputFile);
                //string str = string.Format("{0} 3 {1}", inputFile.FilePath, outputContainerSasUrl);
                string taskCommandLine = String.Format("cmd /c %AZ_BATCH_APP_PACKAGE_APPLICATIONPACKAGE_TEST_01%\\TaskApplication.exe {0} 3 \"{1}\"", inputFile.FilePath, outputContainerSasUrl);
                //string taskCommandLine = @"cmd /c %AZ_BATCH_APP_PACKAGE_APPLICATIONPACKAGE_TEST_01%\\TaskApplication.exe {0} 3 \"{1}\"";
                //string taskCommandLine =@"cmd /c %AZ_BATCH_APP_PACKAGE_APPLICATIONPACKAGE_TEST_01%\\TaskApplication.exe " + string.Format("{0} 3 {1}", inputFile, outputContainerSasUrl);
